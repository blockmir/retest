    ...
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "    {\n"
                                        "      \"address\":amount   (numeric) The donate address is the key, the numeric amount in btc is the value\n"
                                        "      ,...\n"
                                        "    }\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
                ...
                "amount: (numeric, required) Enter an amount of DON to convert to zDON\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zDON (zerocoins) into DON. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over DON (change), the wallet can convert it automatically back to zerocoins [true]\n"
