...
void CMasternodeMan::AskForMN(CNode* pnode, CTxIn& vin)
{
    ...
    // ask for the mnb info once from the node that sent don

    LogPrint("masternode", "CMasternodeMan::AskForMN - Asking node for missing entry, vin: %s\n", vin.prevout.hash.ToString());
    pnode->PushMessage("dseg", vin);
    int64_t askAgain = GetTime() + MASTERNODE_MIN_DON_SECONDS;
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "don") { //Masternode Ping
        CMasternodePing don;
        vRecv >> don;

        LogPrint("masternode", "don - Masternode ping, vin: %s\n", don.vin.prevout.hash.ToString());

        if (mapSeenMasternodePing.count(don.GetHash())) return; //seen
        mapSeenMasternodePing.insert(make_pair(don.GetHash(), don));

        int nDoS = 0;
        if (don.CheckAndUpdate(nDoS)) return;
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
            ...
            if (sigTime - pmn->nLastDseep > MASTERNODE_MIN_DON_SECONDS) {
