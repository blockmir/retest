...
int64_t CMasternode::GetLastPaid()
{
    ...
    CScript donayee;
    donayee = GetScriptForDestination(pubKeyCollateralAddress.GetID());
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        ...
        if (masternodePayments.mapMasternodeBlocks.count(BlockReading->nHeight)) {
            ...
            */
            if (masternodePayments.mapMasternodeBlocks[BlockReading->nHeight].HasPayeeWithVotes(donayee, 2)) {
...
bool CMasternodeBroadcast::Create(CTxIn txin, CService service, CKey keyCollateralAddressNew, CPubKey pubKeyCollateralAddressNew, CKey keyMasternodeNew, CPubKey pubKeyMasternodeNew, std::string& strErrorRet, CMasternodeBroadcast& mnbRet)
{
    ...
    CMasternodePing don(txin);
    if (!don.Sign(keyMasternodeNew, pubKeyMasternodeNew)) {
...
bool CMasternodePing::CheckAndUpdate(int& nDos, bool fRequireEnabled)
{
    ...
    if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto()) {
        ...
        // LogPrint("masternode","doning - Found corresponding mn for vin: %s\n", vin.ToString());
        // update only if there is no known ping for this masternode or
        // last ping was more then MASTERNODE_MIN_DON_SECONDS-60 ago comparing to this one
        if (!pmn->IsPingedWithin(MASTERNODE_MIN_DON_SECONDS - 60, sigTime)) {
            std::string strMessage = vin.ToString() + blockHash.ToString() + boost::lexical_cast<std::string>(sigTime);
            ...
            if (mi != mapBlockIndex.end() && (*mi).second) {
                if ((*mi).second->nHeight < chainActive.Height() - 24) {
                    ...
                    // Do nothing here (no Masternode update, no doning relay)
                    // Let this node to be visible but fail to accept doning
