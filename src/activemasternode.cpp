...
bool CActiveMasternode::SendMasternodePing(std::string& errorMessage)
{
    ...
    CMasternodePing don(vin);
    if (!don.Sign(keyMasternode, pubKeyMasternode)) {
    ...
    if (pmn != NULL) {
        if (pmn->IsPingedWithin(MASTERNODE_PING_SECONDS, don.sigTime)) {
            errorMessage = "Too early to send Masternode Ping";
            return false;
        }

        pmn->lastPing = don;
        mnodeman.mapSeenMasternodePing.insert(make_pair(don.GetHash(), don));

        //mnodeman.mapSeenMasternodeBroadcast.lastPing is probably outdated, so we'll update it
        CMasternodeBroadcast mnb(*pmn);
        uint256 hash = mnb.GetHash();
        if (mnodeman.mapSeenMasternodeBroadcast.count(hash)) mnodeman.mapSeenMasternodeBroadcast[hash].lastPing = don;
...
bool CActiveMasternode::EnableHotColdMasterNode(CTxIn& newVin, CService& newService)
{
    ...
    //The values below are needed for signing doning messages going forward
    vin = newVin;
    service = newService;
