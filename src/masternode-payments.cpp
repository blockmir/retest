...
{
    pathDB = GetDataDir() / "donayments.dat";
...
bool CMasternodePaymentDB::Write(const CMasternodePayments& objToSave)
{
    ...
    LogPrint("masternode","Written info to donayments.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodePaymentDB::ReadResult CMasternodePaymentDB::Read(CMasternodePayments& objToLoad, bool fDryRun)
{
    ...
    LogPrint("masternode","Loaded info from donayments.dat  %dms\n", GetTimeMillis() - nStart);
...
void DumpMasternodePayments()
{
    ...
    LogPrint("masternode","Verifying donayments.dat format...\n");
    ...
    if (readResult == CMasternodePaymentDB::FileError)
        LogPrint("masternode","Missing budgets file - donayments.dat, will try to recreate\n");
    else if (readResult != CMasternodePaymentDB::Ok) {
        LogPrint("masternode","Error reading donayments.dat: ");
    ...
    LogPrint("masternode","Writting info to donayments.dat...\n");
...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything -- find the longest chain
        LogPrint("donayments", "Client not synced, skipping block payee checks\n");
        return true;
    }
...
void CMasternodePayments::ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnget") { //Masternode Payments Request Sync
        ...
        LogPrint("donayments", "mnget - Sent Masternode winners to peer %i\n", pfrom->GetId());
        ...
        if (masternodePayments.mapMasternodePayeeVotes.count(winner.GetHash())) {
            LogPrint("donayments", "mnw - Already seen - %s bestHeight %d\n", winner.GetHash().ToString().c_str(), nHeight);
        ...
        if (winner.nBlockHeight < nFirstBlock || winner.nBlockHeight > nHeight + 20) {
            LogPrint("donayments", "mnw - winner out of range - FirstBlock %d Height %d bestHeight %d\n", nFirstBlock, winner.nBlockHeight, nHeight);
            return;
        }
...
bool CMasternodePayments::IsScheduled(CMasternode& mn, int nNotBlockHeight)
{
    ...
    CScript donayee;
    donayee = GetScriptForDestination(mn.pubKeyCollateralAddress.GetID());
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments)
        if (payee.nVotes >= nMaxSignatures && payee.nVotes >= DONAYMENTS_SIGNATURES_REQUIRED)
            nMaxSignatures = payee.nVotes;

    // if we don't have at least 6 signatures on a payee, approve whichever is the longest chain
    if (nMaxSignatures < DONAYMENTS_SIGNATURES_REQUIRED) return true;
    ...
    BOOST_FOREACH (CMasternodePayee& payee, vecPayments) {
        ...
        if (payee.nVotes >= DONAYMENTS_SIGNATURES_REQUIRED) {
...
void CMasternodePayments::CleanPaymentList()
{
    ...
    while (it != mapMasternodePayeeVotes.end()) {
        ...
        if (nHeight - winner.nBlockHeight > nLimit) {
            LogPrint("donayments", "CMasternodePayments::CleanPaymentList - Removing old Masternode payment - block %d\n", winner.nBlockHeight);
...
bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
{
    ...
    if (n > DONAYMENTS_SIGNATURES_TOTAL) {
        //It's common to have masternodes mistakenly think they are in the top 10
        // We don't want to print all of these messages, or punish them unless they're way off
        if (n > DONAYMENTS_SIGNATURES_TOTAL * 2) {
            strError = strprintf("Masternode not in the top %d (%d)", DONAYMENTS_SIGNATURES_TOTAL * 2, n);
            LogPrint("masternode","CMasternodePaymentWinner::IsValid - %s\n", strError);
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    if (n == -1) {
        LogPrint("donayments", "CMasternodePayments::ProcessBlock - Unknown Masternode\n");
        return false;
    }

    if (n > DONAYMENTS_SIGNATURES_TOTAL) {
        LogPrint("donayments", "CMasternodePayments::ProcessBlock - Masternode not in the top %d (%d)\n", DONAYMENTS_SIGNATURES_TOTAL, n);
        return false;
    }
